...
export function FormBuilderProvider({ renderers, children }: FormBuilderProviderProps) {
  const parent = useContext(FieldRendererContext);
  const merged = { ...parent, ...renderers };

  return <FieldRendererContext.Provider value={merged}>{children}</FieldRendererContext.Provider>;
}
