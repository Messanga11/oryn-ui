...
describe('FormBuilderProvider', () => {
  it('provides field renderers to descendants', async () => {
    const { FormBuilderProvider, useFieldRenderers } = await import('../../index');
    ...
    function Consumer() {
      ...
      return Renderer ? (
        <Renderer field={{} as never} config={{} as never} />
      ) : (
        <div>no renderer</div>
      );
    }
  ...
  it('merges parent and child providers (child overrides)', async () => {
    ...
    function Consumer() {
      ...
      return (
        <div>
          <div data-testid="text">{renderers.text ? 'has-text' : 'no-text'}</div>
          <div data-testid="select">{renderers.select ? 'has-select' : 'no-select'}</div>
        </div>
      );
    }
